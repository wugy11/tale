...
public class FinanceService {
	...
	public void saveFinance(Finance finance) {
		...
		if (null != expenseTime)
			finance.setExpense_time(DateKit.toUnix(finance.getExpenseTime()));
		if (null == id) {
			finance.setCreate_time(DateKit.nowUnix());
	...
	public Map<String, Object> statisticLineData() {
		...
		sql.append("select strftime('%Y-%m', datetime(expense_time,'unixepoch','localtime')) month_str, ")
				...
				.append("group by category, month_str order by month_str");
		...
		listMap.forEach(map -> xAxisDataList.add(String.valueOf(map.get("month_str"))));

		Map<String, Object> resMap = CollectionKit.newHashMap();
		resMap.put("xAxisData", CollectionKit.newArrayList(xAxisDataList));
